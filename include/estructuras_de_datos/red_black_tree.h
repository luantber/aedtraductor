...
public:
	...
	//----------------
	void rotacion_derecha(NodoRB<T>*& p);
	void rotacion_izquierda(NodoRB<T>*& p);
...
template<class T>
void red_black_tree<T>::rotacion_derecha(NodoRB<T>*& p){
...
template<class T>
void red_black_tree<T>::rotacion_izquierda(NodoRB<T>*& p){
	NodoRB<T>**d_father=get_d_father(p);
	//padres
	p->m_pFather=(*d_father)->m_pFather;
	(*d_father)->m_pFather=p;
...
NodoRB<T>** red_black_tree<T>::get_d_father(NodoRB<T>* p){
	NodoRB<T>** father;
	if(p->m_pFather==m_pRoot) return father=&(m_pRoot); 
	return father=&(p->m_pFather->m_pFather->m_pSon[der_izq(p->m_pFather)]); 
}
